...
const (
	address = "grpc.covidero.ml:50051"
)
...
func conectar_server(wri http.ResponseWriter, req *http.Request) {
	...
	if err != nil {
		...
		log.Fatalf("No se puede retornar la informacion: %v", err)
	}

	log.Printf("Respuesta Servidor: %v", ret.GetInfo())
	json.NewEncoder(wri).Encode("Se ha almacenado la informacion")
...
func main() {
	...
	fmt.Println("Cliente levantado en el puerto 8000")
